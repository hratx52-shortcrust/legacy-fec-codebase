...
const QAserver = "http://localhost:3000";
...
const getQA = (id = 1) => {
  return axios.get(`${QAserver}/qa/questions?product_id=${id}`);
};
...
const getSpecificAnswers = (questionId) => {
  return axios.get(`${QAserver}/qa/questions/${questionId}/answers`);
};

const askQuestion = (id, text, name, email) => {
  return axios.post(`${QAserver}/qa/questions`, {
    product_id: id,
    body: text,
    name: name,
    email: email,
...
const answerQuestion = (questionId, text, name, email, photos = []) => {
  return axios.post(`${QAserver}/qa/questions/${questionId}/answers`, {
    body: text,
    name: name,
    email: email,
    photos: photos,
...
const markQAsHelpful = (questionId) => {
  return axios.put(`${QAserver}/qa/questions/${questionId}/helpful`);
};

const reportQuestion = (questionId) => {
  return axios.put(`${QAserver}/qa/questions/${questionId}/report`);
};

const markAnsAsHelpful = (answerID) => {
  return axios.put(`${QAserver}/qa/answers/${answerID}/helpful`);
};

const reportAns = (answerID) => {
  return axios.put(`${QAserver}/qa/answers/${answerID}/report`);
};
